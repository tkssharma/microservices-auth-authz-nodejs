import { BadRequestException, Controller, Get, HttpStatus, Param, Post, Req } from '@nestjs/common';
import {
...
export class AuthController {
  ...
  public async authenticate(@Req() req: Request): Promise<any> {
    ...
    try {
      const isAuthenticate = await this.authService.authenticate(authToken);
      return { isAuthenticate, status: HttpStatus.OK };
    } catch (error) {
